...
#filename:run_diffueraser.py
def main():

    ## input params
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_video', type=str, default="Validation/video_0.mp4", help='Path to the input video')
    parser.add_argument('--input_mask', type=str, default="Validation/mask_0.mp4" , help='Path to the input mask')
    ...
    parser.add_argument('--trained_path', type=str, default="weights/diffuEraser" , help='Path to DiffuEraser')
    parser.add_argument('--propainter_model_dir', type=str, default="weights/propainter" , help='Path to priori model')
    args = parser.parse_args()
    
    if not os.path.exists(args.save_path):
        os.makedirs(args.save_path)
    priori_path = os.path.join(args.save_path, "priori2.mp4")                        
    output_path = os.path.join(args.save_path, "diffueraser_result2.mp4") 
