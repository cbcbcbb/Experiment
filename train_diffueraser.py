...
from dataset import VideoMaskDataset
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default="/data1/cb/DiffuEraser/weights/stable-diffusion-v1-5",
        help="Path to pretrained model or model identifier from huggingface.co/models.",
    )
    parser.add_argument(
        "--brushnet_model_name_or_path",
        type=str,
        default="/data1/cb/DiffuEraser/weights/diffuEraser",
...
def validation(
    ...
    validation_prompts = ""
    validation_images = "/data1/cb/DiffuEraser/Validation/video.mp4"
    validation_masks = "/data1/cb/DiffuEraser/Validation/mask.mp4"
    validation_ppp= "/data1/cb/DiffuEraser/Validation/prior.mp4"
    ...
    print("Compose Start")
    default_fps = 10
    writer = cv2.VideoWriter(f"/data1/cb/DiffuEraser/Validation/step_{step}.mp4", cv2.VideoWriter_fourcc(*"mp4v"),
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if  global_step in [10,100,500,700] or (accelerator.is_main_process and (global_step+1) % 500 == 0 and global_step!=0):
            ...
            with accelerator.accumulate(unet):
                with torch.no_grad():
                    #prepare latents
                    ppp_values = batch["ppp_values"].to(dtype=weight_dtype)
